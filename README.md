...
当前设置页提供 Gemini、OpenAI、Anthropic、DeepSeek、Qwen、xAI、OpenRouter、Groq、Mistral AI、Together AI、SiliconFlow、Moonshot、智谱 GLM、火山方舟 Doubao 等预设，也支持填写自定义 OpenAI 兼容 `chat/completions` 端点。除 Gemini 与 Anthropic 原生接口外，其余预设均按 OpenAI 兼容格式调用。
...
- `host_permissions` for `x.com` / `twitter.com`: 限定内容脚本只在这些站点工作。
- `host_permissions` for model API hosts: 允许后台 service worker 直接请求用户配置的第三方模型 API。
